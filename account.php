...
$result = mysql_query("SELECT * FROM quiz ORDER BY date DESC") or die('Error');
...
while($row = mysql_fetch_array($result)) {
    ...
    $time = $row['time'];
	$eid = $row['eid'];
$q12=mysql_query("SELECT score FROM history WHERE eid='$eid' AND email='$email'" )or die('Error98');
...
$sn=@$_GET['n'];
$total=@$_GET['t'];
$q=mysql_query("SELECT * FROM questions WHERE eid='$eid' AND sn='$sn' " );
...
$q=mysql_query("SELECT * FROM options WHERE qid='$qid' " );
...
$q=mysql_query("SELECT * FROM rank WHERE  email='$email' " )or die('Error157');
while($row=mysqli_fetch_array($q) )
{
$s=$row['score'];
...
if(@$_GET['q']== 2) 
{
$q=mysql_query("SELECT * FROM history WHERE email='$email' ORDER BY date DESC " )or die('Error197');
...
if(@$_GET['q']== 3) 
{
$q=mysql_query("SELECT * FROM rank  ORDER BY score DESC " )or die('Error223');
...
$e=$row['email'];
$s=$row['score'];
$q12=mysql_query("SELECT * FROM user WHERE email='$e' " )or die('Error231');
...
$c++;
echo '<tr><td style="color:#99cc32"><b>'.$c.'</b></td><td>'.$name.'</td><td>'.$gen.'</td><td>'.$col.'</td><td>'.$s.'</td><td>';
}
echo '</table></div></div>';}
?>
